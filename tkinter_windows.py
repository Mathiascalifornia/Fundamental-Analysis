...
class MainWindows:
    ...
    def main(self , App):      
        ...
        def __get_entries(ticker=ticker , path=path):
            ...
            path_ = path.get()
            name_ = name.get()
            App(language=language_ , ticker=ticker_ , path_to_save=path_ , company_name=name_).main()
            root.quit()


        button = Button(root , text='Analyse / Analysis' , command=__get_entries , width=21 , height=0 , borderwidth=1).grid(row=4 , column=1)
        root.mainloop()  